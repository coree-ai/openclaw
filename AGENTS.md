...
The coree MCP server runs via `npx` and provides the tools below. Always call
`session_context()` at the start of each session to load prior context, and
`search()` before each task.
